...
<body>
	...
	<div class="page-wrapper">
			...
			<c:forEach items="${requestScope.videoList }" var="video">
			<p style="text-align: center;font-family: 宋体;font-weight: bold;font-size: 24px;color:#00F">车辆${video.car.license }工作环境实时数据</p> 
			<div class="row" style="margin-top: 2%">
				...
				<div class="videodiv" id="${video.id}">
					...
					<p align="center">																											
										...
										<button type="button" class="btn btn-primary" onMouseover="startPtz(3,0,'${video.serialNumber }');" onMouseout="stopPtz(3,'${video.serialNumber }');">向右</button>										
										<br/><img class="box5"  src="img/littercar.png" width="10%" height="10%" />										
										<%-- <label class="sensordata0">${video.carId}</label> --%>										
										<label class="sensordata0">${video.car.license}</label> 
										<button onclick="editVideo(${video.id},'${video.car.license }','${video.serialNumber }')" class="btn btn-white btn-sm" data-toggle="modal" data-target="#editVideoModal">
											<span class="glyphicon glyphicon-pencil" aria-hidden="true"></span>编辑
										</button>
										<button onclick="deleteVideo(${video.id},'${video.car.license }');" class="btn btn-white btn-sm" data-toggle="modal" data-target="#deleteModal">
					...
					<p align="left">	
										...
										<c:if test="${item.key==video.carId}">
											<table  style="font-size:18;text-align:left;border:5px solid #0088dd;">
										
											<c:forEach items="${ item.value }" var="it"> <br/>	
												
												
												<tr >													 
													    <td style="font-weight: bold">编号：</td>
													    <td></td>
													    <td style="color:blue;">${it.serialNumber }</td>
													    <td style="padding:5px;font-weight: bold">类型：</td>
													    <td></td>
													    <td style="color:blue;">${it.sensorType.type}</td>
													    <td ></td>
													    <td style="padding:5px;font-weight: bold">监测状态：</td>
													    <td></td>
													    <td style="padding:5px；color:blue;">
														<c:if test="${it.status ==0}">正常</c:if>
														<c:if test="${it.status ==1}">异常</c:if>
														</td>
														<td style="padding:5px;font-weight: bold">监测值：</td>
													    <td style="padding:5px ;width:100px;font-weight: bold">
													    <c:if test="${it.sensorType.type=='硫化氢传感器'||it.sensorType.type=='氨气传感器' }">
													    <input type="text" style="width:40px; height:20px;color:blue;" id='${it.id}' name="sensorId" style="width: 20%;" readonly /></td>
											    		</c:if>
													    <c:if test="${it.sensorType.type=='GPS传感器'||it.sensorType.type=='温湿度传感器' }">
													    温度(℃)<input type="text" style="width:65px; height:20px;color:blue;" id='${it.id}' name="sensorId" style="width: 20%;" readonly /></td>
											    		</c:if>
													    <td style="padding:5px;width:100px;font-weight: bold">
														<c:if test="${it.sensorType.type=='GPS传感器'||it.sensorType.type=='温湿度传感器' }">
														湿度(%)<input style="width:65px; height:20px;color:blue;" id='${it.id}value2' style="width: 20%;" readonly />
														</c:if>
														</td>
														<td></td>
														<td style="padding:5px;">
														<c:if test="${it.sensorType.type=='氨气传感器'||it.sensorType.type=='硫化氢传感器' }">
															<button class="btn btn-sm btn-info" onclick="javascript:showRealTimeData(${it.id },'${it.sensorType.type}')">实时数据</button>
														</c:if>
														</td>													  
												</tr>
